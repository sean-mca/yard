...
///
/// Returns the config field descriptors this provider accepts, plus
/// optional source/sink type arrays for validation union (D-03).
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct SchemaResponse {
    /// Config fields the provider understands.
    pub fields: Vec<SchemaField>,
    /// Source types this provider supports (e.g. `["databricks_table"]`).
    /// Core unions these with `SUPPORTED_SOURCE_TYPES` during validation.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub supported_source_types: Option<Vec<String>>,
    /// Sink types this provider supports.
    /// Core unions these with `SUPPORTED_SINK_TYPES` during validation.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub supported_sink_types: Option<Vec<String>>,
}
