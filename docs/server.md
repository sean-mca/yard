...
- **Cross-origin dev (`dx serve` on a different port from the API):** the cookie path does NOT work in this shape regardless of client-side opt-ins. `SameSite=Strict` on `yard_session` blocks the cookie from being sent on any cross-origin request (the strictest level — the browser does not even include the cookie on top-level navigations from a different origin). The supported dev path is `Authorization: Bearer $YARD_API_TOKEN` (header-based) optionally combined with `YARD_API_AUTH_DISABLED=1` (loopback bypass). The WASM `reqwest` calls do thread `credentials: include` through the browser fetch (BL-02 fix) so future cookie attribute relaxations (e.g. `SameSite=Lax` on a cookie scoped to a parent domain) would work without re-touching the WASM client; today's `SameSite=Strict` cookie still won't ride cross-origin.
